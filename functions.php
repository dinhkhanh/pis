...
<li <?php comment_class();?> id="li-comment-<?php comment_ID(); ?>">
  <article id="comment-<?php comment_ID();  ?>" class="comment">
    <footer class="comment-meta">
      <div class="comment-author vcard">
        <?php  printf(__('%1$s %2$s'), sprintf('<span class="fn">%s</span>', get_comment_author_link()), sprintf('<span class="date_fn"><a href="%1$s"><time pubdate datetime="%2$s">%3$s</time></a></span>', esc_url(get_comment_link($comment->comment_ID)), get_comment_time('c'), sprintf(__('%1$s at %2$s'), get_comment_date(), get_comment_time())));
...
if (!function_exists('save_extra_user_profile_fields')):
    function save_extra_user_profile_fields($user_id) {
        ...
        update_usermeta($user_id, 'dk_twitter', $_POST['twitter']);
        update_usermeta($user_id, 'dk_facebook', $_POST['facebook']);
        update_usermeta($user_id, 'dk_linkedin', $_POST['linkedin']);
        update_usermeta($user_id, 'dk_rss', $_POST['rss']);
        update_usermeta($user_id, 'dk_tumblr', $_POST['tumblr']);
        update_usermeta($user_id, 'dk_flickr', $_POST['flickr']);
        update_usermeta($user_id, 'dk_gplus', $_POST['gplus']);
        update_usermeta($user_id, 'dk_youtube', $_POST['youtube']);
        update_usermeta($user_id, 'dk_skype', $_POST['skype']);
        update_usermeta($user_id, 'dk_zingme', $_POST['zingme']);
        update_usermeta($user_id, 'dk_pinterest', $_POST['pinterest']);
        update_usermeta($user_id, 'dk_twitline', $_POST['twitline']);
    }
endif;
